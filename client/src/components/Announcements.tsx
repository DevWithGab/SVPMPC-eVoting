...
export const Announcements: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-20 sm:pt-32 pb-20 sm:pb-32 px-3 sm:px-6 lg:px-8 bg-[#fcfcfd]">
      <div className="container mx-auto max-w-7xl">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-6 sm:gap-12">
            <div className="w-full lg:max-w-3xl">
              <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                ...
                <span className="text-[8px] sm:text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] sm:tracking-[0.5em]">Central Communication Node</span>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 w-full lg:w-auto">
              <div className="flex bg-white border border-gray-100 p-1 sm:p-1.5 shadow-sm rounded-none w-full lg:w-auto">
                  ...
                  <button
                    ...
                    className={`px-3 sm:px-6 py-1.5 sm:py-2.5 text-[8px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex-1 sm:flex-none ${
                      filterPriority === p 
                        ...
                        : 'text-gray-400 hover:text-coop-darkGreen'
                    }`}
                  >
                    {p}
                  </button>
        ...
        {loading ? (
          <motion.div 
            className="py-40 text-center bg-white border border-gray-200 rounded-none"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-sm font-black text-gray-400 uppercase tracking-[0.3em]">Loading Broadcasts...</p>
            </div>
          </motion.div>
        ) : filteredAnnouncements.length === 0 ? (
          <motion.div 
            className="py-40 text-center bg-white border border-dashed border-gray-200 rounded-none"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
          >
            <Megaphone size={64} className="mx-auto text-gray-200 mb-6" />
            <p className="text-sm font-black text-gray-400 uppercase tracking-[0.3em]">No Broadcasts Synchronized</p>
          </motion.div>
        ) : (
          <motion.div 
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mt-12 sm:mt-16 md:mt-20 scroll-mt-40">
              {(filterPriority === 'ALL' ? previousAnnouncements : filteredAnnouncements).map((ann, idx) => (
                <div 
                  ...
                  className="group relative bg-white border border-gray-100 p-4 sm:p-6 md:p-10 rounded-none shadow-sm hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] hover:border-coop-green transition-all duration-500 flex flex-col cursor-pointer overflow-hidden animate-slideUp z-10"
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div className="flex justify-between items-start mb-4 sm:mb-6 md:mb-10 gap-2">
                      <div className={`px-2 sm:px-3 py-0.5 sm:py-1 text-[7px] sm:text-[8px] font-black uppercase tracking-widest border ${
                        ann.priority === 'HIGH' ? 'bg-red-50 text-red-500 border-red-500/20' : 
                        ann.priority === 'MEDIUM' ? 'bg-coop-yellow/10 text-coop-green border-coop-yellow/20' : 'bg-gray-50 text-gray-400 border-gray-200'
                      }`}>
                        {ann.priority}
                      </div>
                      <span className="text-[7px] sm:text-[8px] font-mono text-gray-300 uppercase tracking-widest">ID-{ann.id.slice(-4).toUpperCase()}</span>
                    </div>

                    <h3 className="text-lg sm:text-xl md:text-2xl font-black text-coop-darkGreen uppercase tracking-tighter leading-tight mb-2 sm:mb-4 group-hover:text-coop-green transition-colors line-clamp-2">
                      {ann.title}
                    </h3>
                    <p className="text-xs sm:text-sm text-gray-500 leading-relaxed font-medium mb-4 sm:mb-6 md:mb-10 line-clamp-3">
                      {ann.content}
                    </p>

                    <div className="mt-auto pt-3 sm:pt-4 md:pt-8 border-t border-gray-50 flex items-center justify-between">
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <span className="text-[7px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest">{ann.date}</span>
                      </div>
      ...
      {selectedAnnouncement && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-6 animate-fadeIn pt-20 sm:pt-32 pb-6 sm:pb-0">
          <div className="absolute inset-0 bg-coop-darkGreen/80 backdrop-blur-2xl" onClick={closeDetailedView}></div>
          
          <div className="relative w-full max-w-3xl bg-white rounded-none shadow-[0_80px_150px_-30px_rgba(0,0,0,0.5)] border border-white/20 animate-scaleIn overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[85vh]">
            ...
            <div className="p-4 sm:p-10 md:p-16 flex-grow overflow-y-auto custom-scrollbar">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 mb-8 sm:mb-12">
                ...
                <div className="h-px w-6 sm:w-8 bg-gray-100"></div>
                <span className="text-[7px] sm:text-[10px] font-mono text-gray-300 uppercase tracking-widest">Broadcast-Ref#{selectedAnnouncement.id.toUpperCase()}</span>
                <button 
                  onClick={closeDetailedView}
                  className="ml-auto p-2 sm:p-4 bg-gray-50 hover:bg-gray-100 transition-colors text-gray-400 hover:text-coop-darkGreen"
              ...
              <h2 className="text-2xl sm:text-4xl md:text-5xl font-black text-coop-darkGreen uppercase tracking-tighter leading-none mb-6 sm:mb-10">
                {selectedAnnouncement.title}
              </h2>

              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mb-8 sm:mb-16 pb-8 sm:pb-12 border-b border-gray-100">
                <div className="flex items-center gap-2 sm:gap-4">
                  <div className="p-2 sm:p-3 bg-gray-50 text-coop-yellow rounded-none">
                    <Calendar size={14} className="hidden sm:block" />
                    <Calendar size={12} className="sm:hidden block" />
                  </div>
                  <div>
                    <p className="text-[7px] sm:text-[8px] font-black text-gray-400 uppercase tracking-widest mb-0.5 sm:mb-1">Date Sent</p>
                    <p className="text-xs sm:text-sm font-bold text-coop-darkGreen">{selectedAnnouncement.date}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 sm:gap-4">
                  <div className="p-2 sm:p-3 bg-gray-50 text-coop-yellow rounded-none">
                  ...
                  <div>
                    <p className="text-[7px] sm:text-[8px] font-black text-gray-400 uppercase tracking-widest mb-0.5 sm:mb-1">Authorized Author</p>
                    <p className="text-xs sm:text-sm font-bold text-coop-darkGreen">{selectedAnnouncement.author}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 sm:gap-4">
                  <div className="p-2 sm:p-3 bg-gray-50 text-coop-yellow rounded-none">
                  ...
                  <div>
                    <p className="text-[7px] sm:text-[8px] font-black text-gray-400 uppercase tracking-widest mb-0.5 sm:mb-1">Integrity</p>
              ...
              <div className="prose prose-lg max-w-none">
                <p className="text-sm sm:text-lg md:text-xl text-gray-600 leading-relaxed font-medium whitespace-pre-wrap">
                  {selectedAnnouncement.content}
                </p>
              </div>

              <div className="mt-12 sm:mt-20 p-4 sm:p-10 bg-coop-green/5 border-l-4 border-coop-green flex gap-3 sm:gap-6">
                ...
                <p className="text-xs sm:text-sm text-coop-darkGreen font-semibold leading-relaxed italic">
            ...
            {/* Modal Action Bar */}
            <div className="p-4 sm:p-10 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-8">
              <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto justify-center sm:justify-start">
                ...
                <span className="text-[8px] sm:text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] sm:tracking-[0.3em]">Status: Registry Synchronized</span>
              </div>
              <div className="flex gap-2 sm:gap-4 w-full sm:w-auto">
                <button className="flex-1 sm:flex-none px-10 py-4 border border-gray-200 text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all flex items-center justify-center gap-3">
