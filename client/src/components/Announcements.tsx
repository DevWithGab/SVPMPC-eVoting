...
export const Announcements: React.FC = () => {
  ...
  const fetchAnnouncements = async () => {
    // Picks the best available date field and returns it as a YYYY-MM-DD string
    const getAnnouncementDate = (ann: any): string => {
      const rawDate = ann.date || ann.createdAt || new Date().toISOString();
    ...
    try {
      ...
      // Step 1: Shape each raw API object into the Announcement type
      const announcements: Announcement[] = data.map((ann: any) => ({
        id: ann._id || ann.id,
        title: ann.title || '',
        content: ann.content || '',
        priority: ann.priority || 'LOW',
        date: getAnnouncementDate(ann),
        author: ann.author || 'Unknown'
      }));

      // Step 2: Sort so the newest announcement appears first
              ...
              <div className={`border p-1 sm:p-1.5 shadow-sm rounded-none w-full lg:w-auto transition-colors duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'}`}>
                {['ALL', 'HIGH', 'MEDIUM', 'LOW'].map((p) => (
                  <button
                    ...
                    className={`px-3 sm:px-6 py-1.5 sm:py-2.5 text-[8px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex-1 sm:flex-none ${filterPriority === p
                ...
                >
                  ...
                  <div className="relative z-10 grid lg:grid-cols-12 gap-4 sm:gap-8 md:gap-12 items-center">
                    <div className="lg:col-span-8">
                      ...
                      >
                        <span className={`px-2.5 sm:px-4 py-1 sm:py-1.5 text-[7px] sm:text-[9px] font-black uppercase tracking-widest ${latestAnnouncement.priority === 'HIGH' ? 'bg-red-500' :
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div className="flex justify-between items-start mb-4 sm:mb-6 md:mb-10 gap-2">
                      <div className={`px-2 sm:px-3 py-0.5 sm:py-1 text-[7px] sm:text-[8px] font-black uppercase tracking-widest border transition-colors duration-300 ${ann.priority === 'HIGH' ? 'bg-red-50 text-red-500 border-red-500/20' :
      ...
      {selectedAnnouncement && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-6 animate-fadeIn pt-20 sm:pt-32 pb-6 sm:pb-0">
          ...
          <div className={`relative w-full max-w-3xl rounded-none shadow-[0_80px_150px_-30px_rgba(0,0,0,0.5)] border animate-scaleIn overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[85vh] transition-colors duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-white/20'}`}>
            ...
            <div className={`h-2 w-full ${selectedAnnouncement.priority === 'HIGH' ? 'bg-red-500' :
            ...
            <div className="p-4 sm:p-10 md:p-16 flex-grow overflow-y-auto custom-scrollbar">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 mb-8 sm:mb-12">
                <span className={`px-2.5 sm:px-4 py-1 sm:py-1.5 text-[7px] sm:text-[9px] font-black uppercase tracking-widest ${selectedAnnouncement.priority === 'HIGH' ? 'bg-red-500 text-white' :
