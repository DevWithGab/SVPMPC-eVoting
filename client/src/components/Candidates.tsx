...
import { candidateAPI, positionAPI, electionAPI } from '../services/api';
...
export const Candidates: React.FC<PositionsProps> = ({ electionId, onNavigate }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!targetElectionId) {
          ...
          targetElectionId = activeElection._id || activeElection.id;
        }
        ...
        const filteredPositions = Array.isArray(positionsData) 
          ? positionsData.filter(position => {
              ...
              return posElectionId === targetElectionId;
            })
          : [];
        ...
        const filteredCandidates = Array.isArray(candidatesData)
          ? candidatesData.filter(candidate => {
              ...
              return candElectionId === targetElectionId;
            })
          : [];
    ...
    return (
      <div className="min-h-screen pt-32 pb-32 px-4 bg-[#fcfcfd] flex items-center justify-center">
        <div className="text-center max-w-md">
          <p className="text-coop-darkGreen font-black text-xl mb-4">Error Loading Positions</p>
    ...
    return (
      <div className="min-h-screen pt-32 pb-32 px-4 bg-[#fcfcfd] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-coop-darkGreen font-black">Loading positions...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
            <div className="max-w-3xl">
              <h1 className="text-6xl md:text-8xl font-black text-coop-darkGreen tracking-tighter leading-[0.85] uppercase mb-8">
                <span className="text-coop-green">Positions</span>
              </h1>
              <p className="text-xl text-gray-500 font-medium leading-relaxed max-w-xl border-l-4 border-coop-green/10 pl-8">
                Review all available positions and their respective nominees for the current election cycle.
              </p>
            </div>
          </div>
        </header>

        {!hasActiveElection || positions.length === 0 ? (
          <div className="text-center py-20 bg-white border border-gray-100">
            {!hasActiveElection ? (
              <>
                <p className="text-gray-500 font-medium text-lg">No active election</p>
                <p className="text-gray-500 font-medium text-lg">No positions to show</p>
              </>
            ) : (
              <p className="text-gray-500 font-medium text-lg">No positions found for this election.</p>
            )}
          </div>
        ) : (
          <div className="space-y-8">
            {positions.map((position) => {
              const positionCandidates = candidates.filter(c => {
                const candPosId = c.positionId?._id || c.positionId;
                return candPosId === (position._id || position.id);
              });
              
              return (
                <div key={position._id || position.id} className="bg-white border border-coop-green/10 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  {/* Position Header */}
                  <div className="bg-gradient-to-r from-coop-darkGreen to-coop-green p-6 text-white">
                    <div className="flex items-center gap-3 mb-2">
                      <Briefcase size={24} />
                      <h2 className="text-3xl font-black uppercase tracking-tight">{position.title}</h2>
                    </div>
                    {position.description && (
                      <p className="text-green-100 text-sm ml-9">{position.description}</p>
                    )}
                  </div>
                  ...
                  <div className="p-6">
                    {positionCandidates.length === 0 ? (
                      <p className="text-gray-400 text-center py-8">No nominees for this position</p>
                    ) : (
