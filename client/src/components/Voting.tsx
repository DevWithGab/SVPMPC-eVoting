...
export const Voting: React.FC<{ onNavigate?: (page: PageView) => void }> = ({ onNavigate }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Use the freshly fetched election ID to avoid stale state issues
      const freshElectionId = activeElections[0]._id || activeElections[0].id;
      ...
      for (const position of positions) {
        ...
        if (uniqueCandidateIds.length > 0) {
          ...
          for (const candidateId of uniqueCandidateIds) {
            try {
              const result = await voteAPI.castVote(candidateId, freshElectionId);
              results.push(result);
            } catch (err) {
          ...
          try {
            const result = await voteAPI.castAbstainVote(freshElectionId, position.id);
            results.push(result);
          } catch (err) {
          ...
          {posCandidates.map((c) => {
            ...
            return (
              <button
                ...
                className={`w-full group relative overflow-hidden flex items-center justify-between p-8 rounded-none border-2 transition-all duration-300 shadow-sm ${isSelected
          ...
          {posCandidates.map((c) => {
            ...
            return (
              <div
                ...
                className={`group relative rounded-none border-2 cursor-pointer transition-all duration-500 flex flex-col overflow-hidden stagger-item ${isSelected
